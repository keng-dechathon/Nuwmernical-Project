...
import { Outputbox,MatrixBox, random_bg_color, Topic, Flexbox, Matrix1, Matrix2, Matrix3, MatrixButtonBox, MatrixInputBox } from '../Components/Style-Component';
...
export default function Jacobi() {
    ...
    function jacobi() {
        ...
        console.log(row);
        while (true) {
            let i = 0, eps = 99999999;
            if (!A._data[i][i]) return;
            while (i < MatrixB.length) {
                ...
                x_new = (MatrixB[i] - mul + (mii * MatrixC[i])) / mii;
                eps = Math.abs((x_new - MatrixC[i]) / x_new);
                if (eps <= err) check[i] = 1;
                MatrixC[i] = x_new.toFixed(7);
                ee[i] = eps;
                i++;
            }
            a = '[';
    ...
    return (
        <>
            ...
            {(!Error &&showAns) && <Table columns={columns} dataSource={data} style={{ marginTop: '20px', border: '1px solid black' }} />}
